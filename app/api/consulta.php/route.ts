...
export async function GET(request: NextRequest) {
  ...
  if (!cpf) {
    return NextResponse.json({ status: 400, statusMsg: "Informe o CPF" }, { status: 400 })
  }

  const token = process.env.CPF_API_TOKEN
  if (!token) {
    return NextResponse.json(
      { status: 500, statusMsg: "Configure CPF_API_TOKEN nas variaveis de ambiente" },
      { status: 500 }
    )
  }
  ...
  if (!apiResp || !apiResp.ok) {
    return NextResponse.json(data, { status: apiResp?.status || 500 })
  }
